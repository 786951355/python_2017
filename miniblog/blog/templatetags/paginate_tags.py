...
def paginate(context, object_list, page_count):# context是Context 对象，object_list是你要分页的对象，page_count表示每页的数量
    ...
    try:
        ...
        context['page_list'] = pages[-1] + 1# +1的原因是为了方便判断，在模板文件中将会体会到其作用。
    except IndexError:
        context['page_first'] = 1# 发生异常说明只有1页
        context['pate_last'] = 2# 1 + 1 后的值
...
def get_left(current_page, left, num_pages):
    ...
    if current_page == 1:
        ...
        l.sort()
        return l
    l = [i for i in range(current_page, current_page - left, -1) if i - 1 > 1]
    l.sort()
    return l
